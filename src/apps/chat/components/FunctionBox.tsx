...
export default function FunctionBox({ onClose, onOpenSettings, onOpenBeautify }: FunctionBoxProps) {
  const items: FunctionItem[] = [
    { id: 'voice', label: '语音通话', icon: <PhoneCall size={28} weight="duotone" /> },
    {
      id: 'settings',
      label: '设置',
    ...
    { id: 'thinking', label: '思考强度', icon: <Gauge size={28} weight="duotone" /> },
    ...
    <div className="funcbox-overlay" onClick={onClose}>
      <div className="funcbox" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="funcbox__grid">
          {items.map((fn) => (
            <div key={fn.id} className="funcbox__item" onClick={() => fn.action?.()}>
